...
output_file = 'model-ablation-talk.png'
...
row_names = {
    ...
    'noqueryrewrites': 'No Rewrites',
    'defaultweights': r'No Learning',
...
for (i, col) in enumerate(cols):
    ...
    cname = col_names[col]
    ax.set_title(cname, fontsize=20, y=1.0)
    pos = arange(len(rows))
    ...
    ax.errorbar(val, pos, xerr=errs, elinewidth=1.0, color='black', fmt=' ', capsize=0)
    ax.set_yticks(pos)
    ...
    ax.xaxis.set_tick_params(width=1.0, color='gray')
    ...
    #ax.text(-.005, -1.65, '0')
    ax.axvline(x=baseline_values[col], ymin=0, ymax=1, color='black', lw=1.0)   
